...
write.csv(df, "Surface_Sample_Results_no_reps.csv")
...
#################################################################################
# Transform 1st Order Concentration data 
df_1stOrder_Conc <- df %>%
  select(Sample, Site,  Conc_NonFiber, Conc_Fiber) %>%
  pivot_longer(cols = c( Conc_NonFiber, Conc_Fiber), 
               names_to = "Particle_Type", values_to = "Concentration")
#################################################################################
#Transform Concentration data All, NF, TRWPs
df_long_Con <- df %>%
  select(Sample, Site, Conc_Total, Conc_NonFiber, Conc_TRWP) %>%
  pivot_longer(cols = c(Conc_Total, Conc_NonFiber,  Conc_TRWP), 
               names_to = "Particle_Type", values_to = "Concentration")
#################################################################################
# Transform Concentration data all morphologies
df_long <- df %>%
  select(Sample, Site, Conc_fiber, Conc_fiberbun, Conc_film, Conc_foam, Conc_sphere, Conc_fragment, Conc_pellet, Conc_TRWP) %>%
  pivot_longer(cols = c(Conc_fiber, Conc_fiberbun, Conc_film, Conc_foam, Conc_sphere, Conc_fragment, Conc_pellet, Conc_TRWP), 
               names_to = "Particle_Type", values_to = "Concentration")
#################################################################################
# Transform 1st Order Composition data 
df_1stOrder_Perc <- df %>%
...
# set site order, sort rows by site then sample
...
ggplot(df_1stOrder_Conc, aes(x = Sample, y = Concentration, fill = Particle_Type)) +
  ...
  theme_minimal() +
  labs(title = "Count-based MP Concentration: Surface Sediments (0-5 cm)",
       x = NULL, y = expression(Concentration~"(n/g dw sediment)"),
       fill = "Particle Type") +
  scale_fill_manual(
...
# View results
print(dunn_results, n=30)

write.csv(dunn_results,"surf_sig_conc_kw_dunn_results.csv")
