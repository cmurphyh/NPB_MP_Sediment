...
# Read dataset
df <- read_csv('Intertidal_Sed_MP_SHP_v4.csv')

# Transform 1st Order Concentration data into long format for ggplot
df_1stOrder_Conc <- df %>%
  select(Sample, Site, Location, Depth, Conc_NonFiber, Conc_Fiber) %>%
...
df_1stOrder_Conc <- df_1stOrder_Conc %>%
  mutate(mark_star = ifelse(
    Location == "UNB-2" & Depth %in% c('10-15cm', '15-20cm', '20-25cm', '25-30cm', '30-35cm', '35-40cm', '40-45cm', '45-50cm', '50-55cm') & Particle_Type == "Conc_Fiber",
    "*", ""
  ))
...
p <- ggplot(df_1stOrder_Conc, aes(x = Depth, y = Concentration, fill = Particle_Type)) +
  ...
  theme_bw() +
  labs(title = "Count-based MP Concentration: Intertidal Sediments",
       x = NULL, y = expression(Concentration~"("*n^1~"MP"~g^{-1}~"Sediment dw"*")"),
       fill = "Particle Type") +
  scale_fill_manual(
...
p<- ggplot(df_All_Comp, aes(x = Depth, y = Composition * 100, fill = Particle_Type)) +
  ...
  labs(title = "Count-based Morphological MP Composition",
       x = "Site", y = "Morphological Count Composition [%]",
       fill = "Morphology") +
  scale_fill_manual(
