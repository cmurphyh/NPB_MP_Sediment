...
df <- read.csv("UNB1_plot.csv") #replicates removed and gaps extrapolated
...
# Load and tidy the data
df_BDLOI <- read.csv("UNB1_BDLOI_Results.csv") #replicates removed and gaps extrapolated
...
################################################################################
#(5)
df <- read_csv("UNB1_age_depth_conc_v3.csv")

df$mean <- as.numeric(df$mean)

#df <- rbind.data.frame(df[c(1:45,47),]) #v1

df <- rbind.data.frame(df[c(1:53,55),]) #v2

# Plot 1: Conc_Total [n/kg dw]
p5a <- 
  ggplot(df, aes(x = `Median_n/g/yr` , y = depth, group=group)) +
  geom_line(color = "#007200", size = 1.5) +
  geom_line(aes(x = `Lower_n/g/yr`), color = "gray", linetype = "dashed", size = 1) +
  geom_line(aes(x = `Upper_n/g/yr`), color = "gray", linetype = "dashed", size = 1) +
  #coord_cartesian(ylim = c(50, 0)) +
...
p5 <- ggplot(df, aes(x = `Conc_Total [n/g dw]` , y = depth, group=group)) +
  ...
  coord_cartesian(ylim = c(50, 0)) +
  scale_y_reverse(expand = c(0, 0)) +
  scale_x_continuous(position = "top", limits = c(0, 0.5)) +
  #scale_y_continuous(position = "right") +# Depth increases downward
  #scale_x_continuous( limits = c(0, 500),
                    #  position = "top") +
 # scale_y_continuous(limits = c(1952.2, 2021.7), 
...
p6 <- ggplot(df, aes(x = `Av_Conc_Total [mg/g dw]`, y = depth, group = group)) +
  ...
  scale_y_continuous(
    ...
    sec.axis = dup_axis(
      breaks = c(50, 45, 40, 35, 30, 25, 20, 15, 10, 5, 0),
      labels = c("1942.9", "1952.2", "1962.3", "1969.8", "1976.6", "1983.3", 
                 "1990.1", "1995.6", "2002", "2010", "2021.7"), 
      name = "Year"
    )
  ) +
  labs(
...
text_plot <- ggplot() +
  annotate("text", x = x_pos, y = 26, label = "[9.0 yrs]", size = 4) +
  annotate("text", x = x_pos, y = 35, label = "[10.1 yrs]", size = 4) +
  annotate("text", x = x_pos, y = 43, label = "[7.5 yrs]", size = 4) +
  annotate("text", x = x_pos, y = 50, label = "[6.8 yrs]", size = 4) +
  annotate("text", x = x_pos, y = 59, label = "[6.7 yrs]", size = 4) +
  annotate("text", x = x_pos, y = 67, label = "[6.8 yrs]", size = 4) +
  annotate("text", x = x_pos, y = 75, label = "[5.6 yrs]", size = 4) +
  annotate("text", x = x_pos, y = 84, label = "[6.4 yrs]", size = 4) +
  annotate("text", x = x_pos, y = 91, label = "[7.9 yrs]", size = 4) +
  annotate("text", x = x_pos, y = 100, label = "[11.7 yrs]", size = 4) +
